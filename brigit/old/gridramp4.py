...
# fmax_vec = np.flip(np.round(np.arange(3.7, 4.4+f_resolution, f_resolution), 2))
...
for fmax in fmax_vec:
    for dfdt in dfdt_vec:
        ...
        cmd = "python2 run_my_yelmo.py -l -f -a output/ramp/GridRamp4/1"
        cmd+= " \&tf_corr_ant=\"ronne=0.25 ross=0.2 pine=-0.5\" \&hysteresis_proj=\"time_end=110e3\" \&hyster=\"dt_init=5e3"
