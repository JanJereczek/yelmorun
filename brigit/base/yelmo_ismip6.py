...
def autofolder(params,outfldr0):
    ...
    for p in params[:-2]:           # ommit restart file in name as sometimes path very long!
...
def makejob(params,out,wtime,executable,suffix="",auto=False,force=False,edit=False,submit=False,case="none"):
    ...
    o1_in  = "par/yelmo_ismip6_Antarctica_Rtip.nml"
    ...
    # Get path of ismip parameter file and copy it 
...
def submitjob(outfldr,executable):
    ...
    cmd_job = "cd {} && sbatch submit_ismip6.sh".format(outfldr)
...
def main():
    ...
    outfldr    = 'output/ramp/'  # Default output folder is just the outbase
    auto       = False
    force      = False
